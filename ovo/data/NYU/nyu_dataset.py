...
class NYUDataset(Dataset):
    def __init__(
        ...
        self.split = split
        self.root = os.path.join(root, "NYU" + self.split)
        self.base_dir = os.path.join(
            preprocess_root, "base", "NYU" + self.split)
        self.mask_dir = os.path.join(
            preprocess_root, "nyu_masked", "NYU" + self.split)

        # add total preprocess root here, NOTE that all needed info are save in it already
        self.all_prep_dir = all_prep_dir
        print("all_prep_dir: ", self.split, self.all_prep_dir)
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            all_prep_gt_path = os.path.join(self.all_prep_dir, name + ".pkl")
            with open(all_prep_gt_path, "rb") as handle:
                all_prep_data = pickle.load(handle)

            data["lseg_2d"] = all_prep_data["lseg_2d"]
