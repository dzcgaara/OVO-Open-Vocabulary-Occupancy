...
def _read_rle(rle_filename):  # 0.0005s
    ...
    cam_pose = np.fromfile(fid, np.float32, 16).reshape(
        (4, 4))  # Read camera pose
    vox_rle = (
...
def _downsample_label(label, voxel_size=(240, 144, 240), downscale=4):
    ...
    for i in range(small_size[0] * small_size[1] * small_size[2]):
        ...
        if zero_count > empty_t:
            label_downscale[x, y,
                            z] = 0 if zero_count_0 > zero_count_255 else 255
        else:
            label_i_s = label_bin[
...
def main(config: DictConfig):
    ...
    for split in ["train", "test"]:
        ...
        base_dir = os.path.join(
            config.NYU_preprocess_root, "base", "NYU" + split)
