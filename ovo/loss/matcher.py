...
class HungarianMatcher(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            tgt_mask = targets[b]["masks"][b].to(out_mask)
            ...
            # exit()
            tgt_mask = F.interpolate(tgt_mask[:, None], size=out_mask.shape[-3:], mode="nearest")
            ...
            # Final cost matrix
            # print('self.cost_dice ', self.cost_dice)
            # print('cost_dice', cost_dice)
            # print('cost_dice shape', cost_dice.shape)
            # print(cost_mask.shape, cost_class.shape, cost_dice.shape)
            cost_class = torch.reshape(cost_class, (cost_mask.shape))
            C = (
                self.cost_mask * cost_mask
                + self.cost_class * cost_class
                + self.cost_dice * cost_dice # int * (100, 50)
            )
