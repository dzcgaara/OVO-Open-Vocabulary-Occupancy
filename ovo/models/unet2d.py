...
class UNet2D(nn.Module):
    ...
    def build(cls, **kwargs):
        ...
        "/root/.cache/torch/hub/rwightman_gen-efficientnet-pytorch_master", basemodel_name, pretrained=True, source='local'
        )
        # basemodel = torch.hub.load(
        #     "rwightman/gen-efficientnet-pytorch", basemodel_name, pretrained=True
        # )
        #basemodel.load_state_dict(torch.load('/root/.cache/torch/hub/checkpoints/tf_efficientnet_b7_ns-1dbc32de.pth'))
        print("Done.")
...
if __name__ == '__main__':
    ...
    flops = FlopCountAnalysis(model, inputs=(torch.randn(1, 3, 480, 640),))
    print(flops.total())
